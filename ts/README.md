...
It depends only on the Web Crypto API (`globalThis.crypto.subtle`) — no framework, no storage, no product dependencies — so it runs unchanged in browsers, Node.js (≥ 20), Deno, and Bun.

## Why this exists

The protocol is implemented in production across three languages (Go, TypeScript, Kotlin) inside Vernam Mail. This module is the public, vendor-neutral TypeScript reference for the envelope-field layer. It exists primarily to **demonstrate that the specification is language-agnostic**: it produces byte-identical output to the [Go reference](../go/envelope.go) for the same inputs.
...
It does **not** implement the hybrid ML-KEM-1024 + X25519 key wrapping (`Wrap`/`Unwrap` in the Go reference). That is scheduled as a funded cross-language deliverable; see [../ROADMAP.md](../ROADMAP.md) milestone 3.
